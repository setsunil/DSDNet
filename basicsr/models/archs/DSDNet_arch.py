...
if __name__ == '__main__':
    ...
    net = DSDNet_plus(in_nc=in_nc, out_nc=out_nc, nf_p=nf_p, nf_d=nf_d, max_nc=max_nc, n_iter=n_iter, padding_mode=padding_mode, dropout_rate=dropout_rate)
    # import arch_util as util
    # # from torchprofile import profile_macs
    # from thop import profile, clever_format

    # input = torch.randn(1, 3, 256, 256)
    # kernel = torch.randn(1, 35, 35)

    # macs = profile(net, inputs=(input, kernel))
    # # macs = profile_macs(net, (input, kernel))
    # print(clever_format(macs, "%.3f"))
    # print(macs)

    # timer
    device = torch.device('cuda:0')
    inputs = [torch.randn(1, 3, 512, 512).to(device), torch.randn(1, 3, 288, 288).to(device), torch.randn(1, 3, 256, 256).to(device), torch.randn(1, 3, 280, 280).to(device), torch.randn(1, 3, 228, 344).to(device)]
    kernel = torch.randn(1, 31, 31).to(device)
    net.to(device).eval()
    import time
