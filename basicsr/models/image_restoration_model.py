...
from basicsr.models.archs.DSDNet_arch import bconv
...
class NonBlindImageRestorationModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(NonBlindImageRestorationModel, self).__init__(opt)
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('pixel_opt'):
            pixel_type = train_opt['pixel_opt'].pop('type')
            self.deg_loss = train_opt['pixel_opt'].pop('deg_loss')
            cri_pix_cls = getattr(loss_module, pixel_type)
            self.cri_pix = cri_pix_cls(**train_opt['pixel_opt']).to(
                self.device)
        else:
    ...
    def feed_data(self, data, is_val=False):
        self.lq = data['lq'].to(self.device)
        self.kl = data['kl'].to(self.device)
        if 'gt' in data:
            self.gt = data['gt'].to(self.device)
        if 'noise' in data:
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        preds = self.net_g(self.lq, self.kl)
        if not isinstance(preds, list):
            preds = [preds]
        ...
        if self.cri_pix:
            ...
            # degradation loss
            if self.deg_loss:
                fake_L = bconv(self.output, self.kl, 'replicate') + self.noise
                l_pix += 0.01 * self.cri_pix(fake_L, self.lq)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                if j >= n:
                    j = n
                pred = self.net_g(self.lq[i:j], self.kl[i:j])
                if isinstance(pred, list):
                    pred = pred[-1]
                outs.append(pred.detach().cpu())
                i = j
...
class DSD(NonBlindImageRestorationModel):
    """Base Deblur model for single image deblur."""
    ...
    def __init__(self, opt):
        ...
        if load_path is not None:
            self.load_network(self.net_g, load_path,
                              self.opt['path'].get('strict_load_g', True), param_key=None)
