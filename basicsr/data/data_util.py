...
def triple_paths_from_folder(folders, keys, filename_tmpl):
    """Generate triple paths from folders.

    Args:
        folders (list[str]): A list of folder path. The order of list should
            be [input_folder, gt_folder, kl_folder].
        keys (list[str]): A list of keys identifying folders. The order should
            be in consistent with folders, e.g., ['lq', 'gt', 'kl'].
        filename_tmpl (str): Template for each filename. Note that the
            template excludes the file extension. Usually the filename_tmpl is
            for files in the input folder.
    ...
    assert len(folders) == 3, (
        'The len of folders should be 3 with [input_folder, gt_folder, kl_folder]. '
        f'But got {len(folders)}')
    assert len(keys) == 3, (
        'The len of keys should be 3 with [input_key, gt_key, kl_key]. '
        f'But got {len(keys)}')
    input_folder, gt_folder, kl_folder = folders
    input_key, gt_key, kl_key = keys

    input_paths = sorted(list(scandir(input_folder)))
    gt_paths = sorted(list(scandir(gt_folder)))
    kl_paths = sorted(list(scandir(kl_folder)))
    assert len(input_paths) == len(gt_paths) == len(kl_paths), (
        f'{input_key}, {gt_key} and {kl_key} datasets have different number of images: '
        f'{len(input_paths)}, {len(gt_paths)}, {len(kl_paths)}.')
    paths = []
    for idx in range(len(gt_paths)):
        gt_path = gt_paths[idx]
        basename, ext = osp.splitext(osp.basename(gt_path))
        kl_path = kl_paths[idx]
        ...
        assert input_name in input_paths, (f'{input_name} is not in '
                                           f'{input_key}_paths.')
        gt_path = osp.join(gt_folder, gt_path)
        kl_path = osp.join(kl_folder, kl_path)
        paths.append(
            dict([(f'{input_key}_path', input_path),
                  (f'{gt_key}_path', gt_path),
                  (f'{kl_key}_path', kl_path)
                  ]))
    return paths
