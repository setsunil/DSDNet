...
from models.loss import HEMLoss
from models.loss import SynergyLoss

logger = logging.getLogger('base')


class ADDNNModel(BaseModel):
    def __init__(self, opt):
        super(ADDNNModel, self).__init__(opt)
        ...
        # parameters
        self.nb = opt['network_G']['nb']
        self.c  = opt['network_G']['out_nc']

        if self.is_train:
            self.netG.train()
            self.SS = train_opt['self_supervised']
            # loss
            ...
            if loss_type == 'l1':
                ...
                self.cri_pix = HEMLoss().to(self.device)
            elif loss_type == 'ks':
                self.cri_pix = SynergyLoss().to(self.device)
            else:
    ...
    def optimize_parameters(self, step):
        ...
        if self.SS:
            ...
            l_pix += self.l_pix_w * 0.1 * F.l1_loss(self.fake_L, self.var_L)#one-layer loss
